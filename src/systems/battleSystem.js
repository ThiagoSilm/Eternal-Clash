...
const EFFECT_DEFINITIONS = {
  ...
  "eff005": { "id": "eff005", "name": "Heal All", "type": "onTurnEnd", "valueBase": 0.50, "valuePerLevel": 0.05, "level": 10, "xpBase": 800, "xpMax": 800, "desc": "Heals 50% of all allies' max HP.", "action": "let count = 0; owner.field.forEach(t=>{ if(t.hp > 0) { const heal = t.maxHp * 0.5; t.hp += heal; if(t.hp > t.maxHp) t.hp = t.maxHp; count++; } }); if(count > 0) pushLog('💚 Cura em Área: '+count+' aliados curados em 50%.');" },
  ...
  "eff023": { "id": "eff023", "name": "Bravery", "type": "onTurnStart", "valueBase": 0.20, "valuePerLevel": 0.03, "level": 10, "xpBase": 350, "xpMax": 350, "desc": "Increases your attack by 20% when starting the turn.", "action": "let count = 0; owner.field.forEach(c => { c.attack *= 1.20; count++; }); if(count > 0) pushLog('✨ Coragem: Ataque Buffado para '+count+' aliados.');" },
  ...
  "eff026": { "id": "eff026", "name": "Holy Aura", "type": "onTurnEnd", "valueBase": 0.15, "valuePerLevel": 0.03, "level": 10, "xpBase": 400, "xpMax": 400, "desc": "Heals all living allies by 15% per turn.", "action": "let count = 0; owner.field.forEach(t=>{ if(t.hp>0){ const heal = t.maxHp*0.15; t.hp+=heal; if(t.hp>t.maxHp) t.hp=t.maxHp; count++; } }); if(count > 0) pushLog('✨ Aura Sagrada curou '+count+' aliados em 15%.');" },
  "eff027": { "id": "eff027", "name": "Dark Link", "type": "onAttackStart", "valueBase": 0.50, "valuePerLevel": 0, "level": 10, "xpBase": 1000, "xpMax": 1000, "desc": "Sacrifices 1 ally and gains 50% of its power.", "action": "if(allies.length > 1){ const strongest = allies.slice().sort((a, b) => b.attack - a.attack)[0]; if(strongest && strongest.id !== card.id) { const powerGain = strongest.attack*0.5; card.attack+=powerGain; strongest.hp=0; pushLog('🔪 Link Sombrio: '+strongest.name+' sacrificado para dar +'+powerGain.toFixed(0)+' Ataque.'); } }" },
  ...
  "eff041": { "id": "eff041", "name": "Silence Aura", "type": "onTurnEnd", "valueBase": 0.15, "valuePerLevel": 0.02, "level": 10, "xpBase": 500, "xpMax": 500, "desc": "15% chance to silence all enemies at the end of your turn.", "action": "let count = 0; opponent.field.forEach(e=>{ if(e.hp > 0 && rng.rand() < 0.15) { e.silenced = true; count++; } }); if(count > 0) pushLog('🔇 Aura Silêncio: '+count+' inimigos afetados.');" },
  "eff042": { "id": "eff042", "name": "Burn Aura", "type": "onTurnEnd", "valueBase": 0.10, "valuePerLevel": 0.02, "level": 10, "xpBase": 400, "xpMax": 400, "desc": "Deals 10% attack as burn damage to all enemies at the end of turn.", "action": "let count = 0; const aoeDmg = card.attack * 0.10; opponent.field.forEach(e=>{ if(e.hp > 0) { e.hp = Math.max(0, e.hp - aoeDmg); count++; } }); if(opponent.guardian) { opponent.guardian.hp = Math.max(0, opponent.guardian.hp - aoeDmg); count++; } if(count > 0) pushLog('🔥 Aura Queimação: Dano em '+count+' unidades.');" },
  "eff043": { "id": "eff043", "name": "Vampiric Aura", "type": "onTurnEnd", "valueBase": 0.20, "valuePerLevel": 0.03, "level": 10, "xpBase": 500, "xpMax": 500, "desc": "Heals 20% of damage dealt to all enemies.", "action": "let totalHeal = 0; let count = 0; const drainDmg = card.attack * 0.2; opponent.field.forEach(e=>{ if(e.hp > 0) { e.hp = Math.max(0, e.hp - drainDmg); totalHeal += drainDmg; count++; } }); card.hp = Math.min(card.maxHp, card.hp + totalHeal); if(count > 0) pushLog('🩸 Aura Vampírica: Drenou '+totalHeal.toFixed(0)+' HP de '+count+' inimigos.');" },
  ...
  "eff045": { "id": "eff045", "name": "Poison Aura", "type": "onTurnEnd", "valueBase": 0.08, "valuePerLevel": 0.02, "level": 10, "xpBase": 400, "xpMax": 400, "desc": "Applies poison damage to all enemies every turn.", "action": "let count = 0; opponent.field.forEach(e=>{ if(e.hp > 0) { e.status = e.status||{}; e.status.poison = (e.status.poison||0)+1; count++; } }); if(count > 0) pushLog('🧪 Aura Veneno: Aplicado em '+count+' inimigos.');" },
