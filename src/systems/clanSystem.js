...
// Em uma aplicação real, estas funções usariam um banco de dados ou um arquivo JSON.
// ----------------------------------------------------
...
export function createClan(user, clanName) {
  ...
  user.clanId = newClan.id; // Atualiza o objeto do usuário
...
export function leaveClan(user) {
  ...
  const initialMemberCount = targetClan.members.length;
  ...
  // Se o usuário era o líder, transfere a liderança para o membro mais antigo
  const wasLeader = targetClan.members.some(m => m.userId === user.id && m.role === "LIDER");
  if (wasLeader && targetClan.members.length > 0) {
    const newLeader = targetClan.members.reduce((prev, curr) =>
      (new Date(prev.joinedAt) < new Date(curr.joinedAt) ? prev : curr)
    );
...
/**
 * Obtém informações detalhadas de um clã.
 * @param {string} clanNameOrId O nome ou ID do clã.
 * @returns {string} Mensagem de resposta.
 */
export function getClanInfo(clanNameOrId) {
  ...
  const membersList = targetClan.members
    .sort((a, b) => (b.role === 'LIDER' ? 1 : a.role === 'LIDER' ? -1 : b.donated - a.donated))
...
/**
 * Obtém o ranking global de clãs (TOP 10).
