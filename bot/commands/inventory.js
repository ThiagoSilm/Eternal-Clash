...
import { levelUpCard, burnCardForXp, getCardXPValue, getCardNextLevelXP } from "../../src/systems/xpSystem.js";
...
export default {
  ...
  usage: "[list [filtros] | deck [deckName] | add <idx> [deckName] | remove <idx> [deckName] | clear [deckName] | upgrade <...>]",
  ...
  async execute(message, args, user) {
    ...
    if (subcommand === "list") {
      ...
      if (Object.keys(filters).length > 0) cards = filterCards(cards, filters);
      if (cards.length === 0) return message.reply("⚠️ Nenhuma carta encontrada com os filtros aplicados.");
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          cards.map((c, i) => {
            ...
            return `**${i + 1}. ${template?.name || c.id}** — ${c.rarity}★ — Lv.${c.level}\n*Efeitos:* ${effectsStr}`;
          }).join("\n\n")
        );
    ...
    if (subcommand === "add") {
      ...
      if (!index || index < 1 || index > user.cards.length) return message.reply("❌ Índice inválido.");
    ...
    const embed = new EmbedBuilder()
      ...
      .setColor("#e74c3c")
      .setDescription("Use um dos subcomandos válidos:\n`list`, `deck`, `add`, `remove`, `clear`, `upgrade`");
